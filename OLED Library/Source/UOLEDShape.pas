...
begin
  ...
  begin
    ...
    if newValue< OLEDPotentiometer.minValue then newValue:= OLEDPotentiometer.minValue;
    if newValue> OLEDPotentiometer.maxValue then newValue:= OLEDPotentiometer.maxValue;
...
procedure TOLEDElementOLED.Paint;
    ...
    VAR cl:TColor;
        ...
        r:Trect;
    begin
      if not OLEDPotentiometer.visible then exit;
      with OLEDPotentiometer,Canvas do
      begin
        cl:=CalcButtonColor;
        Brush.Color:=cl;
...
begin
  case shape of
    ...
    tsKnob:with OLEDPotentiometer,Canvas do
              begin
                bm:=getBitmap(BmpSunriseKnob);
                StretchDraw(Rect(0,0,Width,Height),bm);
                StretchDraw(Rect(0,0,Width-2,Height),getRotKnob(value));
              end;
    tsSlider:with OLEDPotentiometer,Canvas do
              begin
